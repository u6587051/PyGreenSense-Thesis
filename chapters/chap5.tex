...
นอกจากนี้ ผู้จัดทำได้เปรียบเทียบผลลัพธ์ที่แสดงผ่าน {\italicfont Webview Dashboard} กับผลลัพธ์ดิบจากการรันไลบรารีผ่าน Terminal เพื่อยืนยันว่าข้อมูลด้านพลังงาน คาร์บอน และรายละเอียดปัญหาที่ตรวจพบใน Extension มีความสอดคล้องกับผลลัพธ์จาก {\italicfont PyGreenSense CLI} โดยตรง ดังแสดงในรูป \ref{fig:ch5_webview_energy_metrics}, รูป \ref{fig:ch5_webview_issue_details} และรายการ \ref{lst:ch5_terminal_library_output} กรณีทดสอบนี้ใช้ไฟล์ \texttt{tests/before.py} ซึ่งตรวจพบปัญหารวม 9 รายการ โดยค่าหลักที่ตรงกัน ได้แก่ carbon emission เท่ากับ $4.107350 \times 10^{-7}$ kg CO$_2$, energy consumption เท่ากับ $1.000 \times 10^{-6}$ kWh, CFP เท่ากับ 43, LOC เท่ากับ 345, emissions rate เท่ากับ 24.98 gCO$_2$eq/kWh และจำนวนการรัน 5 รอบ

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/chap5/webview_energy_metrics.png}
    \captionsetup{name=รูป}
    \caption{ผลลัพธ์ด้านพลังงานและคาร์บอนผ่าน Webview Dashboard}
    \label{fig:ch5_webview_energy_metrics}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/chap5/webview_issue_details.png}
    \captionsetup{name=รูป}
    \caption{รายละเอียดปัญหาที่ตรวจพบผ่านแท็บ Issues ของ Webview Dashboard}
    \label{fig:ch5_webview_issue_details}
\end{figure}

\begingroup
\captionsetup[lstlisting]{format=plain,justification=centering,singlelinecheck=false}
\begin{lstlisting}[
    captionpos=b,
    caption={ผลลัพธ์ด้านพลังงานและคาร์บอนจาก Terminal output ของ PyGreenSense Library},
    label={lst:ch5_terminal_library_output},
...
--------------------------------------------------------------------------------
  ...
  DuplicatedCode (2 issue(s)):
    Line 286: Similar function/method implementations (similarity: 92.5%): Company.generate_payroll_report() (line 286, 8 statements), Company.generate_payroll_report_backup() (line 308, 7 statements). Consider refactoring into a single function.
    Line 327: Similar function/method implementations (similarity: 100.0%): Company.find_managers() (line 327, 4 statements), Company.find_vice_presidents() (line 335, 4 statements), Company.find_interns() (line 343, 4 statements). Consider refactoring into a single function.

  GodClass (1 issue(s)):
    Line 276: Class 'Company' is a God Class: complexity 40 (max: 35), 143 lines (max: 100). Consider refactoring by extract into sub class.

  LongMethod (1 issue(s)):
    Line 363: Method 'generate_annual_report' is too long: LOC: 56 (max: 25), Cyclomatic Complexity: 13 (max: 10). Consider refactoring by extracting smaller methods.

  MutableDefaultArguments (2 issue(s)):
    Line 279: Function '__init__' has a mutable default argument. Consider using None and initializing inside the function.
    Line 286: Function 'generate_payroll_report' has a mutable default argument. Consider using None and initializing inside the function.
...
Tracking carbon emissions for: /Users/tardi9rad3/hym-dev/GitHub/PyGreenSense-Library/tests/before.py
...
[]
[SalariedEmployee(name='Louis', role='manager', vacation_days=25, monthly_salary=5000)]
[HourlyEmployee(name='Tim', role='intern', vacation_days=25, hourly_rate=50, amount=10)]
...
Execution Details:
  Target file: /Users/tardi9rad3/hym-dev/GitHub/PyGreenSense-Library/tests/before.py
  Duration: 0.00 seconds

Energy & Emissions:
  Total energy consumed: 0.000001 kWh
  Carbon emissions: 4.107350e-07 kg CO2
  Emissions rate: 24.98 gCO2eq/kWh
  Region: samut sakhon
  Country: Thailand
...
Code Smell & Carbon Emission Analysis
   ...
   Current Run (Initial):
      Carbon Emission: 4.107350e-07 kg CO2
...
จากการเปรียบเทียบพบว่า Webview สามารถนำค่าจากผลลัพธ์ของไลบรารีมาแสดงในรูปแบบที่อ่านง่ายขึ้น โดยยังคงสาระสำคัญของข้อมูลเดิมไว้ครบถ้วน เช่น จำนวนปัญหา ประเภทของปัญหา เลขบรรทัดที่ตรวจพบ ค่า CFP ค่า LOC ค่าพลังงานที่ใช้ และค่าคาร์บอนที่ปล่อยออกมา การแสดงผลดังกล่าวช่วยเพิ่มความน่าเชื่อถือของ Extension เนื่องจากผู้ใช้สามารถตรวจสอบทั้งค่ามาตรวัดและรายละเอียดของ issue ที่ปรากฏบนหน้ารายงานกับผลลัพธ์จาก Terminal ได้โดยตรง ทั้งนี้ การเปรียบเทียบในส่วนนี้มุ่งยืนยันความสอดคล้องของการส่งต่อและแสดงผลข้อมูล ไม่ใช่การพิสูจน์ความแม่นยำของการวัดคาร์บอนโดยตรง
